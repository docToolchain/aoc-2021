...
fun main(args: Array<String>) {
	...
	var solution1: Int = 0
	var solution2: Int = 0
	var dumboField = mutableListOf<Int>()
	...
	File("day2111_puzzle_input.txt").forEachLine {
		for (i in 0..it.length - 1) {
			dumboField.add(it[i].toString().toInt())
		}
	}
	...
	while (!allDumboFlash || n < 101) {
		...
		while (greaterNine) {  // loop while grid.contains 9
			...
			for (i in 0..99) {
				if (dumboField[i] > 9 && dumboField[i] < 100) {
						...
						for (dx in -1..1) {
							if (!(dx == 0 && dy == 0)) {
								...
								if ((y + dy) >= 0 && (y + dy) <= 9 && (x + dx) >= 0 && (x + dx) <= 9) {
									if (dumboField[(x + dx) + (y + dy) * 10] < 100) {
										dumboField[(x + dx) + (y + dy) * 10] =
											dumboField[(x + dx) + (y + dy) * 10] + 1
									}
									if (dumboField[(x + dx)  + (y + dy) * 10] > 9) {
										greaterNine = true
									}
								}
							}
