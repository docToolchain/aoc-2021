...
Thus, during each step of the game, for each current game state, the code checks
what are the 6 possible new game states and counts how many there are for each
(number of current games times number of die rolls arriving at that score).
